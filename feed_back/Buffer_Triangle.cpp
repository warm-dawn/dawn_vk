...
bool Buffer_Triangle::choose_physical_dev(dawn::vk::Physical_device const& pd_) {
    return true;
}
std::vector<char const*> Buffer_Triangle::instance_layers() {
    return {
        # if defined(_DEBUG)
...
void Buffer_Triangle::init() {
    vertices_size = sizeof(Vertex) * vertices.size();
    Buffer_Triangle_base::init();
...
void Buffer_Triangle::init_surface() {
...
void Buffer_Triangle::rendering() {
...
void Buffer_Triangle::on_resize() {}

void Buffer_Triangle::run() {
    MSG message;
    wnd->show();
    while (false == godie) {
        wnd->update();
...
void test_Buffer_Triangle() {
    Buffer_Triangle triangle;
    triangle.init();
    triangle.run();
}
